...
text_quality_ranges = {"pretrain": (0.0, 1.0), "fine_tune": (0.0, 1.0)}
...
class DocumentDataset(Dataset):
    def __init__(self, file, tokenizer, max_len, doc_config, segment_config, train_cfg):
        ...
        if file.endswith(".json"):
            ...
            self.data = pd.read_parquet(file).to_dict(orient="records")
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.whole_segment:
            ...
            representation_doc_token = encode_document(
                text, self.tokenizer, self.max_len, self.doc_sequences
            )
...
class DocumentDatasetSiamese(DocumentDataset):
    def __init__(self, file, tokenizer, max_len, doc_config, segment_config, train_cfg):
        super().__init__(
            file, tokenizer, max_len, doc_config, segment_config, train_cfg
        )
    ...
    def __getitem__(self, index):
        ...
        return (
            torch.tensor(item["id"], dtype=torch.int),
            representation_doc_token,
            label,
        )
...
class DocumentDatasetForPredict(Dataset):
    def __init__(
        ...
        if file.endswith(".json"):
            ...
            df = pd.read_parquet(file).drop(filter_list)
            df = df[df["dedup_after_lang_score"] > 0.6] 
            languages = df["language"].unique()
            if "yue_Hant" in languages or "zho_Hant" in languages:
                raise NotImplementedError("Unsupported language")
            cn_index = df["lang_after_dedup"].eq("zh") | (
                df["language"].eq("zh") & df["lang_score"].gt(0.6)
            )
            self.data = df[cn_index]
            self.data.loc[:, "text"] = self.data["text"].apply(
                lambda x: convert(x, "zh-cn")
            )
            df[~cn_index].to_json(file.replace('/data/','/scored-data/').replace('.parquet','.jsonl'), orient='records', lines=True)
        else:
            raise NotImplementedError("Unsupported file format")
    ...
    def __getitem__(self, index):
        ...
        text_segments, representation_doc_token = encode_document_with_whole_segment(
            item["text"], self.tokenizer, self.max_len
        )

        text_index = [index] * len(text_segments)

        return item, text_index, text_segments, representation_doc_token
...
class DocumentDatasetForEvaluation(DocumentDataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(self.segment_scales)):
            representation_segment = encode_document(
                text, self.tokenizer, self.segment_scales[i], self.segment_sequences[i]
            )
...
class DocumentDatasetForUST(Dataset):
    ...
    def __init__(self, train_cfg, tokenizer, max_len):
        ...
        self.file_list = [
            file for file in os.listdir(self.train_folder) if file.endswith(".json")
        ]
        self.mode = "sample"
        self.data = None
    ...
    def candidate_data_generate(self):
        selected_file_list = random.sample(
            self.file_list, self.train_cfg.select_file_num
        )
        ...
        random_index = random.sample(
            range(total_num), int(self.train_cfg.candidate_data_num)
        )
        ...
        for file, indexes in index_for_file.items():
            result = pool.apply_async(
                self.collect_data, (self.train_folder + file, indexes)
            )
    ...
    def select_samples(self, ids: list, pseudo_labels: list, confidences: list):
        ...
        input_dict = {
            ids[n]: {"pseudo_label": pseudo_labels[n], "confidence": confidences[n]}
        ...
        self.data = [
            {
                "text": dict_with_id_key[identity]["text"],
        ...
        if self.train_cfg.use_paradigm:
            paradigm_confidence = np.mean(
                np.array([self.data[n]["confidence"] for n in range((len(self.data)))])
            )
            ...
            if self.train_cfg.data_scheme == "paradigm":
                ...
                pos_data = [
                    item for item in self.data if int(item["pseudo_label"]) == 1
                ]
            ...
            select_paradigm_data = [
                {
                    "text": self.paradigm_texts[index],
                    "pseudo_label": 1.0,
